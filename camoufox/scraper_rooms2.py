...
This version is specifically adapted to extract detailed information
for single room listings (habitaciones).
...
class ListingExtractor:
    ...
    def __init__(self):
        # --- MODIFIED SECTION ---
        # Enhanced patterns for room-specific details
        self.listing_patterns = {
            "idealista": {
                ...
                "size": r"(\d+)\s*m²", # Kept for general compatibility, but rare for rooms
    ...
    def extract_single_listing(
        ...
        """Extract data from a single listing, enhanced for rooms."""
        try:
            ...
            # Price Drop Info (Optional)
            original_price_match = re.search(patterns["original_price"], listing_html, re.IGNORECASE)
            if original_price_match:
                listing["original_price_text"] = original_price_match.group(1).strip()

            price_drop_match = re.search(patterns["price_drop_percentage"], listing_html, re.IGNORECASE)
            if price_drop_match:
                try:
            ...
            # Room-specific details from 'item-detail-char' div
            details_char_html_match = re.search(r'<div[^>]*class="[^"]*item-detail-char[^"]*"[^>]*>(.*?)</div>', listing_html, re.DOTALL | re.IGNORECASE)
            if details_char_html_match:
                details_html = details_char_html_match.group(1)

                total_rooms_match = re.search(patterns["total_rooms_in_flat"], details_html, re.IGNORECASE)
                if total_rooms_match:
                    listing["total_rooms_in_flat"] = int(total_rooms_match.group(1))

                flatmates_match = re.search(patterns["flatmates"], details_html, re.IGNORECASE)
                if flatmates_match:
                    listing["flatmates_info"] = flatmates_match.group(1).strip().replace("<span>", " ").replace("</span>", "")

                smoking_policy_match = re.search(patterns["smoking_policy"], details_html, re.IGNORECASE)
                if smoking_policy_match:
                    listing["smoking_policy"] = smoking_policy_match.group(1).strip()

                availability_match = re.search(patterns["availability_date"], details_html, re.IGNORECASE)
                if availability_match:
                    listing["availability_date"] = availability_match.group(1).strip()

            # General size (less common for rooms, but good to have)
